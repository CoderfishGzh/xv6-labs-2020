...
User_kvminit() {
  ...
  // virtio mmio disk interface
  User_kvmmap(page ,VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);

  // CLINT
  // User_kvmmap(page, CLINT, CLINT, 0x10000, PTE_R | PTE_W);

  // PLIC
  User_kvmmap(page, PLIC, PLIC, 0x400000, PTE_R | PTE_W);

  // map kernel text executable and read-only.
  User_kvmmap(page, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);

  // map kernel data and the physical RAM we'll make use of.
  User_kvmmap(page, (uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);

  // map the trampoline for trap entry/exit to
  // the highest virtual address in the kernel.
  User_kvmmap(page, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
